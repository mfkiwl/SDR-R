...
static void * cleanupthread(void * args) {
  ...
  //fclose(outputFile); // not needed, main thread will close
...
*/
   ...
   /*
progstart:
    printf("Start of Program, exittrigger = %d\n",exittrigger);
    if (exittrigger > 0){
        goto exitsuccess;
    }
    else {
...
/*  printf("MAIN:\tStarting Cleanup Thread\n");
    retval = pthread_create(&tid2,NULL,cleanupthread,NULL);
    if (retval != 0) {
        fprintf(stderr,"MAIN:\tAn error occured spawning cleanup thread!\n");
        goto fail1;
    }
    ...
    if (retval != 0) {
        fprintf(stderr,"MAIN:\tError on joining cleanup thread with main\n");
        goto fail1;
    }
    */
    // Close file
